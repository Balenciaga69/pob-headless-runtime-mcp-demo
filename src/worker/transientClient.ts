...
export async function callTransientPobWorker(request: PobWorkerRequest): Promise<unknown> {
    ...
    child.on("close", (code, signal) => {
      finish(() => {
        ...
        if (!trimmedStdout) {
          reject(
            new Error(
              `PoB worker returned no stdout. code=${String(code)} signal=${String(signal)} stderr=${stderr.trim()}`,
            ),
          );
          return;
        }
