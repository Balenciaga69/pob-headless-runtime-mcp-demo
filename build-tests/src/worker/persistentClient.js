...
class PersistentPobWorker {
    ...
    async start() {
        ...
        this.process.once("exit", (code, signal) => {
            this.rejectAll(new Error(`PoB worker exited unexpectedly (code=${String(code)}, signal=${String(signal)}).`));
            this.reset();
        });
        ...
        this.lines.on("line", (line) => {
            ...
            if (!response.ok) {
                pending.reject(new Error(`PoB worker request failed with ${response.error.code}: ${response.error.message}`));
                return;
            }
